...
use Flamarkt\Backoffice\AddTranslations;
use Flamarkt\Backoffice\Content\Dashboard;
use Flamarkt\Backoffice\Middleware\RequireManagerAbility;
...
class BackofficeServiceProvider extends AbstractServiceProvider
{
    public function register()
    {
        ...
        $this->container->bind('flarum.assets.backoffice', function () {
            ...
            // | works as OR because this value is injected into a regex
        ...
        $this->container->bind('flarum.frontend.backoffice', function () {
            ...
            $frontend->content(function (Document $document) {
                $document->appView = 'backoffice::frontend.app';
                $document->layoutView = 'backoffice::frontend.backoffice';
            });
    ...
    protected function setDefaultRoute(RouteCollection $routes)
    {
        ...
        if (isset($routes->getRoutes()['GET'][$defaultRoute]['handler'])) {
            ...
            $toDefaultController = $factory->toFrontend('backoffice', Dashboard::class);
        }
